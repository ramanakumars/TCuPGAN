...
class LSTMUNetOneShot(nn.Module):
    gen_type = 'UNet'
    ...
    def __init__(self, hidden_dims=[8, 16, 32], decoder_hidden_dims=None, input_channels=1, output_channels=4):
        ...
        prev_mask_filt = output_channels
        ...
        for i in range(len(hidden_dims)):
            ...
            mask_encoder_layers.append(nn.Conv2d(prev_mask_filt, hidden_dims[i], (3, 3), stride=(2, 2), padding=(1, 1)))
            mask_encoder_layers.append(nn.LeakyReLU(0.2, True))
            mask_encoder_layers.append(nn.InstanceNorm2d(hidden_dims[i]))

            # update the filter value for the next iteration
            prev_filt = hidden_dims[i]
            prev_mask_filt = hidden_dims[i]

        self.encoder_layers = nn.ModuleList(encoder_layers)
        self.mask_encoder_layers = nn.Sequential(*mask_encoder_layers)
    ...
    def forward(self, x, input_mask):
        ...
        mask_encoding = self.mask_encoder_layers(input_mask)
