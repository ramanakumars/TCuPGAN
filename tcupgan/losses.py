...
def fc_tversky(y_true, y_pred, beta=0.7, gamma=0.75, batch_mean=True):
    smooth = 1
    tp = torch.sum(y_true*y_pred, axis=(1, 2))
    fn = torch.sum((1. - y_pred)*y_true, axis=(1, 2))
    fp = torch.sum(y_pred*(1. - y_true), axis=(1, 2))
    tversky = (tp+smooth) /\
...
MSE = lambda y_pred, y_true: torch.mean(torch.sum((y_pred - y_true)**2, axis=(1, 2, 3, 4)))
MAE = lambda y_pred, y_true: torch.mean(torch.sum(torch.abs(y_pred - y_true), axis=(1, 2, 3, 4)))

mink_power = 1
mink = lambda y_pred, y_true: torch.mean(torch.sum(torch.abs(y_pred - y_true)**mink_power, axis=(1, 2, 3, 4)))#**(1./mink_power))

# alias
adv_loss = lambda y_pred, y_true: focal_loss(y_pred, y_true)
gen_loss = mink
