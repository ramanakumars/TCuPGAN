...
class LSTMUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.decoder_layers):
            ...
            #print(x.shape, henc.shape, c.shape)
            x, c = layer(xconc, h=henc, c=c)
...
class EncLSTMUNet(nn.Module):
    gen_type = 'UNet'
    def __init__(self, hidden_dims=[8, 16, 32], bottleneck_dims=[16, 8],
                 input_channels=1, output_channels=4):

        super(EncLSTMUNet, self).__init__()
        ...
        for i in range(len(decoder_hidden_dims) - 1):
            ...
            us2d = nn.Upsample(scale_factor=(2, 2))
            conv2d = nn.Conv2d(hidden_dim * 2,
                                decoder_hidden_dims[i+1], (3, 3),
                                stride=1, padding=1)
            act2d = nn.ReLU(True)
            bn2d = nn.BatchNorm2d(decoder_hidden_dims[i+1])

            decoder_layers.append(nn.Sequential(us2d, conv2d, act2d, bn2d))
    ...
    def forward(self, x):
        ...
        c_new = c.unsqueeze(1).repeat(1, x.shape[1], 1, 1, 1)
        dec_x = x * c_new

        nlayers = len(self.decoder_layers)
        for i, layer in enumerate(self.decoder_layers):
            # skip the x vector across the bottleneck
            xconc = torch.cat([dec_x, xencs[nlayers - i - 1]], dim=2)

            dec_x = time_distribute(xconc, layer)
